            ...
            for global_rank, process in processes.items():
                if process.poll() is None:
                    ...
                    if process.returncode != 0:
                        log.warning(f'Rank {global_rank} crashed with exit code {process.returncode}.')
                        process_has_crashed = True
                        break
                    else:
                        # exited cleanly after successful execution
                        log.info(f'Rank {global_rank} completed execution successfully.')
...
def _aggregate_process_returncode(processes: Dict[int, subprocess.Popen]) -> int:
    for global_rank, process in processes.items():
        process.poll()
        if process.returncode is None:
            log.error(f'Global rank {global_rank} (PID {process.pid}) has not exited yet; returning exit code 1.')
            return 1
        if process.returncode != 0:
            log.error(f'Global rank {global_rank} (PID {process.pid}) exited with code {process.returncode}.')
