...
Thanks for considering contributing to CompTo test your changes locally, run:

1. `make test-cpu`  # run CPU tests
1. `make test-gpu`  # run GPU tests
1. `cd docs && make doctest-cpu`  # run doctests

Some of our checks test distributed training as well. To test these, run:

* `make test-dist-cpu WORLD_SIZE=2`  # run 2-cpu distributed testsues tagged with [good first issue](https://github.com/mosaicml/composer/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) are great options to start contributing.
